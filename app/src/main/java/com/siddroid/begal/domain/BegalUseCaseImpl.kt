...
class BegalUseCaseImpl(val repository: BegalRepository): BegalUseCase {
    override suspend fun invoke(config: BegalConfig): BegalEntityData<BegalDTO> {
    ...
    override suspend fun invoke(count: Int, config: BegalConfig): BegalEntityData<BegalListDTO> {
        ...
        return response.toBegalEntity().also { it.index = getCurrentIndex() }
    }

    override suspend fun invokePrevious(config: BegalConfig): BegalEntityData<BegalDTO> {
    ...
    override suspend fun invokeNext(config: BegalConfig): BegalEntityData<BegalDTO> {
        return if (config.enableMemCache) {
    ...
    private fun Resource<BegalDTO>.toBegalEntity(): BegalEntityData<BegalDTO> {
    ...
    private fun Resource<BegalListDTO>.toBegalEntity(): BegalEntityData<BegalListDTO> {
        return BegalEntityData(
            state = if (status == Resource.Status.ERROR || status == Resource.Status.UNKNOWN || status == Resource.Status.NO_INTERNET) EntityState.ERROR else EntityState.SUCCESS,
            message = message.orEmpty(),
            data = data
